...
release_pattern = %r{(/releases/download/v)\d+\.\d+\.\d+(/)}
abort "Expected three release URLs" unless formula.scan(release_pattern).length == 3

formula.gsub!(release_pattern, "\\1#{version}\\2")
